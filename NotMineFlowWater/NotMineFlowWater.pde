...
void simulate() {
  sources.add(new PVector(0,0));
  for(PVector p:sources){
    mass[(int)p.x][(int)p.y]=new_mass[(int)p.x][(int)p.y]=2;
  }
...
void addPos(){
  ...
  int my = constrain( map_height - int( (mouseY - panel_height) / block_height), 1, map_height );
    PVector vec=new PVector( mx,my);
    if(!sources.contains(vec))sources.add(vec);
  }
}
void draw() {
  background(255);
