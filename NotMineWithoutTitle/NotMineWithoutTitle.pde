...
void setup(){
  ...
  size(1000,1000);
  //noBackground();
  PImage uplo = loadImage("up_lo.gif");
  PImage uphi = loadImage("up_hi.gif");
  PImage updo = loadImage("up_down.gif");
  PImage dolo = loadImage("down_lo.gif");
  PImage dohi = loadImage("down_hi.gif");
  PImage dodo = loadImage("down_down.gif");
  int x1 = bo;
  int y1 = height - bo - is;
  int x2 = bo*2 + is;
  button = new ImageButton(x1, y1, is, is, uplo, uphi, updo);
  button2 = new ImageButton(x2, y1, is, is, dolo, dohi, dodo);
  myFont = loadFont("RotisSanSer-Bold.vlw");
  textFont(myFont,14);
  fx = bo*3 + is*2;
  fy = height - bo - is;
  mods = new Module[num];
...
class ImageButton extends Button
{
  ...
  PImage updo;
  PImage currentimage;
  ImageButton(int _x1, int _y1, int _w, int _h, PImage _uplo, PImage _uphi, PImage _updo)
  {
    x = _x1;
    y = _y1;
    w = _w;
